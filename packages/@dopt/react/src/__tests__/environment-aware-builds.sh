...
echo -e "------------------------------------------------"
NODE_ENV=production pnpm exec rollup --config rollup.test.config.ts --configPlugin typescript; rm -rf ./tmp/types; 
...
echo -e "-------------------------------------------------"
NODE_ENV=development pnpm exec rollup --config rollup.test.config.ts --configPlugin typescript; rm -rf ./tmp/types;
OUTPUT=`node ./src/__tests__/scripts/extract-base-url.js ../../../tmp/index.cjs.js`

if [ $OUTPUT != "http://localhost:7070" ]; then
  echo -e "Expected http://localhost:7070, got $OUTPUT";
  TEST_FAILURES=$((TEST_FAILURES+1))
fi
echo -e "-------------------------------------------------"
